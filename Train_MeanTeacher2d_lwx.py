...
def train(snapshot_path, base_lr, num_classes, batch_size, max_iterations,patch_size,
    ...
    db_val = datasetModelSegwithopencv(valimages, vallabels,
                                       split="val")
    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            #     labs = labs.squeeze(0)
                # labs = labs.permute(2, 0, 1)
                # summed_labs = labs.sum(dim=0)
                # gray_labs = labs.view(1024, 1024, 1)
                # summed_labs = Image.fromarray(gray_labs.numpy())
                # writer.add_image('train/GroundTruth', labs, iter_num)   # 将真实标签图像添加到'train/GroundTruth'下


            if epoch_num >= 50 and iter_num % len(trainloader) == 0:
                model.eval()
                dice_total = 0
                hd95_total = 0
                ...
                for i_batch, sampled_batch in enumerate(valloader):
                    ...
                    for i in range(pred.shape[0]-1):
                        pred[i, :, :] *= i+1
                    pred_background = pred[-1, :, :]
                    pred_background = np.logical_not(pred_background).astype(pred_background.dtype)
                    pred[-1, :, :] = pred_background
                    prediction = np.sum(pred, axis=0)
                    prediction_uint8 = (prediction * 255).astype(np.uint8)
                    save_path = os.path.join(save_dir, 'iter_{}_dice_{}.png'.format(iter_num, round(dice, 4)))
...
if __name__ == "__main__":
    ...
    # 获取数据集
    data_dir = r'/data/cosfs/LWX/STS2024/dataset/Train_292359/train292359.csv'
    ...
    data_dir2 = r'/data/cosfs/LWX/STS2024/dataset/Validation/validation.csv'
    ...
    labeled_num = 29    # 有标签样本数量
    labeled_bs = 2  # 处理有标签样本batch
